...
import { analyzeVideoWithClaude } from "./claude-video-analysis";
...
export async function runPipeline(
  ...
  try {
    ...
    log(`Configuração carregada: ${config.configName}`);

    // Carregar criadores
    const allCreators = readCreators();
    const creators = allCreators.filter((c) => c.category === config.creatorsCategory);
    if (creators.length === 0) throw new Error(`Nenhum criador encontrado para a categoria "${config.creatorsCategory}"`);

    progress.creatorsTotal = creators.length;
    log(`Encontrados ${creators.length} criadores — raspando todos em paralelo`);
    emit();
    ...
    const scrapeResults = await Promise.allSettled(
      creators.map(async (creator) => {
        ...
        addTask({ id: taskId, creator: creator.username, step: "Raspando reels" });

        const reels = await scrapeReels(creator.username, params.maxVideos, params.nDays);
        updateTask(taskId, `Encontrados ${reels.length} reels`);
        ...
        updateTask(taskId, `Top ${topVideos.length} selecionados`);
        log(`@${creator.username}: ${reels.length} reels → top ${topVideos.length} selecionados`);
    ...
    for (const result of scrapeResults) {
      if (result.status === "fulfilled") {
        ...
        const msg = `Erro ao raspar: ${result.reason instanceof Error ? result.reason.message : result.reason}`;
    ...
    log(`Raspagem concluída. ${allTopVideos.length} vídeos para analisar (${VIDEO_CONCURRENCY} workers)`);
    emit();
    ...
    await runWithConcurrency(allTopVideos, VIDEO_CONCURRENCY, async (video) => {
      ...
      try {
        addTask({ id: taskId, creator: video.username, step: "Baixando", views: video.views });

        const videoResponse = await fetch(video.videoUrl);
        if (!videoResponse.ok) throw new Error(`Falha no download: ${videoResponse.status}`);
        const videoBuffer = Buffer.from(await videoResponse.arrayBuffer());
        const contentType = videoResponse.headers.get("content-type") || "video/mp4";

        updateTask(taskId, "Claude analisando com Vision");
        log(`@${video.username} (${label}): Claude analisando (${videoBuffer.length} bytes)`);

        const analysis = await analyzeVideoWithClaude(
          videoBuffer,
          video.thumbnail,
          config.analysisInstruction
        );

        updateTask(taskId, "Claude gerando conceitos");
        log(`@${video.username} (${label}): Claude gerando conceitos`);
        ...
        removeTask(taskId);
        log(`@${video.username} (${label}): concluído`);
        emit();
      } catch (err) {
        removeTask(taskId);
    ...
    // Escrever todos os vídeos novos de uma vez
    ...
    log(`Pipeline concluído! ${progress.videosAnalyzed}/${progress.videosTotal} vídeos analisados, ${progress.errors.length} erros.`);
    emit();
  } catch (err) {
    progress.status = "error";
    const msg = `Erro no pipeline: ${err instanceof Error ? err.message : err}`;
    progress.errors.push(msg);
    log(msg);
    emit();
  }
