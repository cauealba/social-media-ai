    ...
    <div className="space-y-8">
      ...
      <div>
        ...
        <p className="mt-1 text-sm text-muted-foreground">
          Navegue por reels dos competidores analisados com insights de IA
        </p>
      </div>
      ...
      <div className="flex flex-wrap items-center gap-3">
        <Select value={filterConfig} onValueChange={setFilterConfig}>
          ...
          <SelectContent>
            <SelectItem value="all">Todas as Configs</SelectItem>
            {configs.map((c) => (
        ...
        <Select value={sortBy} onValueChange={(v) => setSortBy(v as SortOption)}>
          ...
          <SelectContent>
            ...
            <SelectItem value="date-posted">Data Postado</SelectItem>
            <SelectItem value="date-added">Data Adicionado</SelectItem>
            <SelectItem value="starred">Favoritados Primeiro</SelectItem>
          </SelectContent>
        </Select>
      ...
      {filtered.length === 0 && (
        <div className="glass rounded-2xl p-12 text-center">
          ...
          <h3 className="mt-4 font-semibold">Nenhum vídeo encontrado</h3>
          <p className="mt-1 text-sm text-muted-foreground">
            Execute uma análise de pipeline para gerar resultados, ou ajuste seus filtros.
          </p>
        </div>
      )}
      ...
      <Dialog open={!!modalVideo} onOpenChange={(open) => { if (!open) setModalVideo(null); }}>
        <DialogContent className="max-w-3xl max-h-[90vh] overflow-hidden glass-strong rounded-2xl border-white/[0.08] p-0 gap-0">
          <DialogTitle className="sr-only">
            {modalSection === "analysis" ? "Análise do Vídeo" : "Novos Conceitos"}
          </DialogTitle>
          {modalVideo && (
            <>
              {/* Modal header */}
